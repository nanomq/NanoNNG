...
{
	nng_msg *msg;
	uint8_t  connack[] = { 0x20, 0x18, 0x00, 0x00, 0x15, 0x11, 0x00, 0x00,
		 0x00, 0x3c, 0x27, 0x00, 0x10, 0x00, 0x00, 0x25, 0x01, 0x2a,
		 0x01, 0x29, 0x01, 0x22, 0xff, 0xff, 0x28, 0x01 };
	...
	property_data *session_expiry = mqtt_property_get_value(
	    nng_mqtt_msg_get_connack_property(msg), SESSION_EXPIRY_INTERVAL);
	NUTS_TRUE(session_expiry != NULL);
	NUTS_TRUE(session_expiry->p_value.u32 == 60);
