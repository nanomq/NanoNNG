...
public:
  ...
  inline bool operator==(const Timestamp& _rhs) const {
    return std::tie(milliSeconds) == std::tie(_rhs.milliSeconds);
  }
  inline bool operator<(const Timestamp& _rhs) const {
    return std::tie(milliSeconds) < std::tie(_rhs.milliSeconds);
  }
  ...
  inline std::string toString() const {
    ...
    _aidl_os << "Timestamp{";
    _aidl_os << "milliSeconds: " << ::android::internal::ToString(milliSeconds);
    _aidl_os << "}";
    return _aidl_os.str();
  }
};
