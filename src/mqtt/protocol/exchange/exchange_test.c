...
void test_exchange_init_edge_cases(void)
{
	exchange_t *ex = NULL;
	char **ringBufferName;
	unsigned int caps = 10;
	...
	ringBufferName = nng_alloc(1 * sizeof(char *));
	ringBufferName[0] = nng_alloc(100 * sizeof(char));
	strcpy(ringBufferName[0], "ringBuffer1");

	/* rbsCount >= RINGBUFFER_MAX should fail */
	NUTS_TRUE(exchange_init(&ex, EX_NAME, "topic1", &caps,
	    ringBufferName, &fullOps, RINGBUFFER_MAX) != 0);

	/* name too long (> EXCHANGE_NAME_LEN = 32) should fail */
	char long_name[64];
	memset(long_name, 'a', sizeof(long_name) - 1);
	long_name[sizeof(long_name) - 1] = '\0';
	NUTS_TRUE(exchange_init(&ex, long_name, "topic1", &caps,
	    ringBufferName, &fullOps, 1) != 0);

	/* NULL rbsCaps should fail */
	NUTS_TRUE(exchange_init(&ex, EX_NAME, "topic1", NULL,
	    ringBufferName, &fullOps, 1) != 0);

	/* NULL rbsName should fail */
	NUTS_TRUE(exchange_init(&ex, EX_NAME, "topic1", &caps,
	    NULL, &fullOps, 1) != 0);

	/* rbsName[i] == NULL should fail */
	char *nullName[1] = { NULL };
	NUTS_TRUE(exchange_init(&ex, EX_NAME, "topic1", &caps,
	    nullName, &fullOps, 1) != 0);

	nng_free(ringBufferName[0], 100 * sizeof(char));
	nng_free(ringBufferName, 1 * sizeof(char *));
}

void test_exchange_add_rb(void)
{
	exchange_t *ex = NULL;
	char **ringBufferName;
	unsigned int caps = 10;
	...
	NUTS_TRUE(exchange_add_rb(NULL, NULL) != 0);
	NUTS_TRUE(exchange_add_rb(ex, NULL) != 0);

	/* Add a valid ringBuffer */
	ringBuffer_t *rb = NULL;
	NUTS_TRUE(ringBuffer_init(&rb, 5, RB_FULL_NONE, -1) == 0);
	NUTS_TRUE(rb != NULL);
	strcpy(rb->name, "added_rb");
	NUTS_TRUE(exchange_add_rb(ex, rb) == 0);
	NUTS_TRUE(ex->rb_count == 2);

	NUTS_TRUE(exchange_release(ex) == 0);
}

void test_exchange_get_ringBuffer(void)
{
	exchange_t *ex = NULL;
	char **ringBufferName;
	unsigned int caps = 10;
	...
	/* Existing name should succeed */
	ringBuffer_t *rb = NULL;
	NUTS_TRUE(exchange_get_ringBuffer(ex, "ringBuffer1", &rb) == 0);
	NUTS_TRUE(rb != NULL);

	/* Non-existing name should fail */
	rb = NULL;
	NUTS_TRUE(exchange_get_ringBuffer(ex, "nonExistent", &rb) != 0);
	NUTS_TRUE(rb == NULL);

	NUTS_TRUE(exchange_release(ex) == 0);
}

void test_exchange_handle_msg_null_inputs(void)
{
	exchange_t *ex = NULL;
	char **ringBufferName;
	unsigned int caps = 10;
	...
	/* NULL ex should fail */
	nng_msg *msg = alloc_pub_msg((char *)&topic);
	NUTS_TRUE(exchange_handle_msg(NULL, 0, (void *)msg, NULL) != 0);
	nng_msg_free(msg);

	/* NULL msg should fail */
	NUTS_TRUE(exchange_handle_msg(ex, 0, NULL, NULL) != 0);
...
NUTS_TESTS = {
	...
	{ "Exchange init edge cases", test_exchange_init_edge_cases },
	{ "Exchange release test", test_exchange_release },
	{ "Exchange ringBuffer test", test_exchange_ringBuffer },
	{ "Exchange add ringBuffer test", test_exchange_add_rb },
	{ "Exchange get ringBuffer test", test_exchange_get_ringBuffer },
	{ "Exchange handle msg null inputs", test_exchange_handle_msg_null_inputs },
	{ NULL, NULL },
};