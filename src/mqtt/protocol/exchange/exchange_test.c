...
void test_exchange_init(void)
{
	...
	NUTS_TRUE(exchange_init(NULL, NULL, "topic1", 0, 0, &caps, ringBufferName, &fullOps, 1) != 0);
	NUTS_TRUE(exchange_init(&ex, NULL, "topic1", 0, 0, &caps, ringBufferName, &fullOps, 1) != 0);

	NUTS_TRUE(exchange_init(&ex, EX_NAME, "topic1", 0, 0, &caps, ringBufferName, &fullOps, 1) == 0);
	NUTS_TRUE(ex != NULL);
