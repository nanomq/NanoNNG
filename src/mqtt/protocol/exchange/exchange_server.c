...
static int
get_blf_files_raw(uint32_t sz, char **fnames, char ***file_raws)
{
...
static int
get_blf_files(uint32_t sz, char **fnames, cJSON *obj)
{
	int ret;

	log_info("Ask blf and found.");
	...
	char **file_raws = NULL;
	ret = get_blf_files_raw(sz, fnames, &file_raws);
	if (ret != 0) {
		return -1;
	}
