...
nng_dialer_reload_tls(conf_bridge_node *node, nni_dialer *ndialer)
{
	...
	if (tls->cert_encrypted && tls->key) {
		...
		if (!plain || plainsz == 0) {
			...
			log_info("AES decrypt key %s successfully! %s", tls->keyfile, plain);
			nng_free(tls->key, 0);
			tls->key = plain;
		}
	}
	...
	if (tls->cert_encrypted && tls->cert) {
		...
		if (!plain || plainsz == 0) {
			...
			log_info("AES decrypt cert %s successfully! %s", tls->certfile, plain);
			nng_free(tls->cert, 0);
			tls->cert = plain;
		}
	}
	...
	if (tls->cert_encrypted && tls->ca) {
		...
		if (!plain || plainsz == 0) {
			...
			log_info("AES decrypt ca %s successfully! %s", tls->cafile, plain);
			nng_free(tls->ca, 0);
			tls->ca = plain;
		}
	}
